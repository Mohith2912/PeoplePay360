...
export function success(data, message = "OK", status = 200) {
    return NextResponse.json({ success: true, message, data }, { status });
}
export function failure(message, status = 400, errors) {
    return NextResponse.json({ success: false, message, ...(errors ? { errors } : {}) }, { status });
}
export function handleError(error) {
    if (error instanceof ZodError)
        return failure("Validation failed", 400, error.flatten().fieldErrors);
