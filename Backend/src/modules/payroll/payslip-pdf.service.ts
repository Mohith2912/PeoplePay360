...
function amountInWords(amount: number) { return `${amount.toFixed(2)} currency units`; }
...
export async function generatePayslipPdf(id: string) {
  ...
  const earnings = payslip.lines.filter((line: { category: string }) => ["BASIC", "ALLOWANCE", "GROSS", "REIMBURSEMENT"].includes(line.category));
  const deductions = payslip.lines.filter((line: { category: string }) => line.category === "DEDUCTION");
  const rows = (lines: Array<{ name: string; amount: unknown }>) => lines.map((line) => `<tr><td>${line.name}</td><td>${Number(line.amount).toFixed(2)}</td></tr>`).join("");
  ...
  try { const page = await browser.newPage(); await page.setContent(html, { waitUntil: "load" }); return await page.pdf({ format: "A4", printBackground: true }); }
