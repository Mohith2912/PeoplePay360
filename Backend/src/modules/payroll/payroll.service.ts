...
function resolveContract(contracts: { startDate: Date; endDate: Date | null }[], start: Date, end: Date) {
  const matches = contracts.filter((contract) => contract.startDate <= end && (contract.endDate === null || contract.endDate >= start));
  if (matches.length === 0) throw new BusinessRuleError("No active contract exists for this payroll period");
  if (matches.length > 1) throw new ConflictError("Overlapping active contracts exist for this payroll period");
  return matches[0];
}
...
export async function computePayrun(payrunId: string) {
  return prisma.$transaction(async (tx: any) => {
    ...
    if (payrun.status === "PAID") throw new BusinessRuleError("Paid payruns cannot be recomputed");
    ...
    if (employees.length === 0) throw new BusinessRuleError("No employees are attached to this payrun");
    for (const employee of employees) {
      const contract = resolveContract(employee.contracts, payrun.periodStart, payrun.periodEnd) as typeof employee.contracts[number];
      if (contract.salaryStructureId && contract.salaryStructureId !== payrun.salaryStructureId) throw new BusinessRuleError(`Employee ${employee.employeeCode} is assigned to a different salary structure`);
      const workedDays = employee.attendanceRecords.filter((record: { status: string }) => ["PRESENT", "LATE"].includes(record.status)).length;
      ...
      const values = { periodStart: payrun.periodStart, periodEnd: payrun.periodEnd, workedDays: decimal(workedDays), status: "COMPUTED" as const, grossEarnings: decimal(totals.grossEarnings), grossDeductions: decimal(totals.grossDeductions), netPay: decimal(totals.netPay), totalReimbursement: decimal(totals.totalReimbursement), netTransfer: decimal(totals.netTransfer) };
...
export async function validatePayrun(payrunId: string) {
  ...
  if (!payrun) throw new NotFoundError("Payrun not found");
  const warnings: Array<{ type: "MISSING_BANK_DETAILS" | "MISSING_PAN" | "NO_ACTIVE_CONTRACT" | "OVERLAPPING_CONTRACT"; message: string; employeeId: string; payslipId: string }> = [];
  for (const payslip of payrun.payslips) {
    const employee = payslip.employee;
    if (!employee.bankAccountNumber || !employee.ifscCode) warnings.push({ type: "MISSING_BANK_DETAILS", message: `Missing bank details for ${employee.employeeCode}`, employeeId: employee.id, payslipId: payslip.id });
    if (!employee.panNumber) warnings.push({ type: "MISSING_PAN", message: `Missing PAN for ${employee.employeeCode}`, employeeId: employee.id, payslipId: payslip.id });
    const contracts = await prisma.contract.findMany({ where: { employeeId: employee.id } });
    try { resolveContract(contracts, payrun.periodStart, payrun.periodEnd); } catch (error) { warnings.push({ type: error instanceof ConflictError ? "OVERLAPPING_CONTRACT" : "NO_ACTIVE_CONTRACT", message: error instanceof Error ? error.message : "Invalid contract", employeeId: employee.id, payslipId: payslip.id }); }
  }
...
export async function markPayrunPaid(payrunId: string) {
  ...
  if (!payrun) throw new NotFoundError("Payrun not found");
  if (payrun.status !== "VALIDATED") throw new BusinessRuleError("Only validated payruns can be marked paid");
