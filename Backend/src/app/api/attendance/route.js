...
async function calculate(input) {
    ...
    const status = !input.checkIn || !input.checkOut ? "EXCEPTION" : scheduleDay && input.checkIn.getUTCHours() * 60 + input.checkIn.getUTCMinutes() > minutes(scheduleDay.startTime) ? "LATE" : "PRESENT";
...
export async function GET(request) { try {
    ...
    const data = await prisma.attendanceRecord.findMany({ where: { ...(employeeId ? { employeeId } : {}), ...(status ? { status: status } : {}), ...(startDate || endDate ? { date: { ...(startDate ? { gte: new Date(startDate) } : {}), ...(endDate ? { lte: new Date(endDate) } : {}) } } : {}) }, include: { employee: true }, orderBy: { date: "desc" } });
    return success(data);
}
catch (error) {
...
export async function POST(request) { try {
    ...
    if (!isHr(user.role) && input.employeeId !== user.employee?.id)
        return failure("You can only create your own attendance", 403);
    const calculated = await calculate(input);
    const existing = await prisma.attendanceRecord.findUnique({ where: { employeeId_date: { employeeId: input.employeeId, date: input.date } } });
    if (existing)
        throw new ConflictError("Attendance already exists for this date");
    const result = await prisma.attendanceRecord.create({ data: { employeeId: input.employeeId, date: input.date, checkIn: input.checkIn, checkOut: input.checkOut, workedHours: calculated.workedHours, breakMinutes: calculated.employee.workingSchedule?.days.find((day) => day.dayOfWeek === dayNames[input.date.getUTCDay()])?.breakMinutes, status: calculated.status, notes: input.notes } });
    await audit(user.id, "ATTENDANCE_CREATED", "ATTENDANCE", result.id);
    return success(result, "Attendance recorded", 201);
}
catch (error) {
    return handleError(error);
} }
