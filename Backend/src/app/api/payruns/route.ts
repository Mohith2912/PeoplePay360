...
export async function GET() {
  try {
    ...
    if (!isPayrollReader(user.role)) return failure("Payroll access is not allowed for this role", 403);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      if (!structure || !structure.isActive) throw new NotFoundError("Salary structure not found or inactive");
      const employees = await tx.employee.findMany({ where: { id: { in: input.employeeIds }, status: { not: "TERMINATED" } }, select: { id: true } });
      if (employees.length !== input.employeeIds.length) throw new NotFoundError("One or more selected employees were not found");
      const existing = await tx.payrun.findUnique({ where: { name: input.name } });
      if (existing) throw new ConflictError("A payrun with this name already exists");
