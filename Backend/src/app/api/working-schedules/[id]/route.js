...
import { z } from "zod";
const day = z.object({ dayOfWeek: z.enum(["MONDAY", "TUESDAY", "WEDNESDAY", "THURSDAY", "FRIDAY", "SATURDAY", "SUNDAY"]), startTime: z.string().regex(/^\d{2}:\d{2}$/), endTime: z.string().regex(/^\d{2}:\d{2}$/), breakMinutes: z.number().int().min(0), isWorking: z.boolean() });
const schema = z.object({ name: z.string().min(2), type: z.string().min(1), company: z.string().optional(), days: z.array(day).min(1) });
function hours(days) { const seen = new Set(); return days.reduce((total, item) => { if (seen.has(item.dayOfWeek)) throw new Error(`Duplicate schedule day: ${item.dayOfWeek}`); seen.add(item.dayOfWeek); if (!item.isWorking) return total; const [sh, sm] = item.startTime.split(":").map(Number); const [eh, em] = item.endTime.split(":").map(Number); const shift = eh * 60 + em - sh * 60 - sm; if (shift <= 0 || item.breakMinutes >= shift) throw new Error(`Invalid shift for ${item.dayOfWeek}`); return total + (shift - item.breakMinutes) / 60; }, 0); }
