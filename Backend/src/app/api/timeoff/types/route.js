...
export async function GET() { try {
    await requireAuth();
    return success(await prisma.timeOffType.findMany({ orderBy: { name: "asc" } }));
}
catch (error) {
...
export async function POST(request) { try {
    ...
    return success(await prisma.timeOffType.create({ data: schema.parse(await request.json()) }), "Time-off type created", 201);
}
catch (error) {
    return handleError(error);
} }
=======
export { typesGET as GET, typesPOST as POST } from '@/modules/integration/operations';
