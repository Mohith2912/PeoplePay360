...
export async function GET(request) {
    try {
        ...
        const totalNetSalaryPaid = payslips.reduce((sum, item) => sum + Number(item.netTransfer), 0);
        ...
        const salaryByDepartment = employees.reduce((result, employee) => { const amount = payslips.filter((slip) => slip.employeeId === employee.id).reduce((sum, slip) => sum + Number(slip.netTransfer), 0); const row = result.find((item) => item.department === employee.department); if (row)
            row.totalSalary += amount;
        else
            result.push({ department: employee.department, totalSalary: amount, headcount: 1 }); return result; }, []);
        return success({ kpis: { totalNetSalaryPaid, payslipsGenerated: payslips.length, averageSalary: payslips.length ? totalNetSalaryPaid / payslips.length : 0, approvedTimeOffDays: approvedTimeOff.reduce((sum, item) => sum + Number(item.duration), 0), attendanceHealth: { ...counts, total: totalAttendance, presentPercent: totalAttendance ? counts.present / totalAttendance * 100 : 0, latePercent: totalAttendance ? counts.late / totalAttendance * 100 : 0, absentPercent: totalAttendance ? counts.absent / totalAttendance * 100 : 0 } }, salaryByDepartment, departmentBreakdown: salaryByDepartment, monthlySalaryTrend: [], warnings, attendanceOverview: { present: counts.present, late: counts.late, absent: counts.absent, overtime: 0, missingCheckouts: attendance.filter((record) => !record.checkOut).length, manualEdits: attendance.filter((record) => record.isManuallyCorrected).length, coveragePercent: employees.length ? totalAttendance / employees.length * 100 : 0 }, pendingTimeOffRequests: 0, expiringContracts: 0 });
    }
    catch (error) {
