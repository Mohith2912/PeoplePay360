...
const configs = {
  ...
  allocations: { title: "Leave Allocations", description: "Manage employee leave entitlements and remaining balances.", endpoint: "/api/timeoff/allocations", columns: ["employeeName", "typeName", "allocatedAmount", "takenAmount", "remainingAmount", "status"], fields: [{ name: "employeeId", label: "Employee", source: "employees", required: true }, { name: "timeOffTypeId", label: "Leave type", source: "types", required: true }, { name: "periodStart", label: "Valid from", type: "date", defaultValue: currentYearStart, required: true }, { name: "periodEnd", label: "Valid until", type: "date", defaultValue: currentYearEnd, required: true }, { name: "allocatedAmount", label: "Days / hours to allocate", type: "number", defaultValue: 12, min: 0, required: true }] },
};

const display = (value) => value == null ? "—" : typeof value === "boolean" ? (value ? "Yes" : "No") : String(value).replace("T", " ").replace(".000Z", "");

async function fetchAllPages(endpoint, params = {}) {
  const first = await apiClient.get(endpoint, { params: { ...params, page: 1, limit: 50 } });
  const rows = [...(first.data.data || [])];
  const pages = Math.ceil((first.data.total || rows.length) / 50);
  if (pages > 1) {
    const remaining = await Promise.all(Array.from({ length: pages - 1 }, (_, index) => apiClient.get(endpoint, { params: { ...params, page: index + 2, limit: 50 } })));
    remaining.forEach((response) => rows.push(...(response.data.data || [])));
  }
  return rows;
}
...
export default function OperationsPage({ module }) {
  ...
  const canManageAllocations = canManageLeave(user?.role);
  ...
  async function load() {
    ...
    try {
      ...
      setRows(await fetchAllPages(config.endpoint, { employeeId }));
    } catch (requestError) {
  ...
  useEffect(() => {
    load();
    Promise.all([fetchAllPages("/api/employees"), fetchAllPages("/api/timeoff/types")])
      .then(([employees, types]) => setOptions({ employees: employees.map((item) => ({ value: item.id, label: item.name })), types: types.map((item) => ({ value: item.id, label: item.name })) }))
      .catch(() => {});
  }, [module]);
  ...
  const canCreate = module === "allocations" ? canManageAllocations : isHR || ["requests", "attendance"].includes(module);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <header className="erp-page-header">
        ...
        {canCreate && <button type="button" className="erp-primary-button" onClick={() => setEditing({})}><Plus className="h-4 w-4" /> New record</button>}
      </header>
